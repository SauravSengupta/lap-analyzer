...
> **NOTE (2026-05-10):** This is the **original design spec**, written before implementation. The pipeline has since diverged in important ways — most notably, the track-definition and drift-correction architectures were rebuilt around Google Maps satellite-derived apex pins as the single source of geometric truth. The current architecture and its rationale live in [`/README.md`](README.md). This spec is preserved as a historical record of the original intent and is **not the source of truth** for any of: the track JSON schema, the calibration/anchor approach, the labeler's drift correction, or the meaning of `apex_m`. Use the README for current behavior and use this spec only for the parts that are still aligned (the high-level component decomposition, the OBD/canonical-speed decisions, the long-form parquet schema, and the design principles in the "Design principles" section).
